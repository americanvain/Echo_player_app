...
- **内置离线词典**（7.3 万词，含音标与词形变化，由 MIT 协议的 ECDICT 筛选）：点词直接出释义，不需要网络；每句自动列出难词
- **问 AI**：听读页里直接问关于这句话的问题，服务器接教学 Agent；离线时本机能答词义、音标、难词、译文
...
| 听读 | 上文 → 当前句（**点词直接出释义和音标**，一排小图标可朗读 / 加生词 / 查词典；**长按划过选出卡住的片段**；盲听时逐词盖住）→ 译文 → **本句词汇** → 评分结果 → **问 AI**；下面是播放控制、跟读区、五层定位按钮 |
...
Release 签名与 CI 见 [docs/BUILD.md](docs/BUILD.md)。第三方内容声明见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。技术栈：Kotlin、Jetpack Compose (Material 3)、Room、DataStore、OkHttp、kotlinx-serialization；minSdk 26 / targetSdk 35。
...
- [x] 内置离线词典、本句词汇、问 AI 面板与 `/chat` 契约（v0.3.0）
